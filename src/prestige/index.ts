...
export class Prestige {
  ...
  constructor(options: PrestigeOptions) {
    ...
    const voteManagerConfig: VoteManagerConfig = {
      cloutUrl: options.config.voterGateCloutUrl ?? options.config.ballotGateCloutUrl,
      scarcityUrl: options.config.voterGateScarcityUrl,
    };
    ...
    this.revealManager = new RevealManager(this.store, this.ballotManager);
  ...
  private createBallotGate(): BallotGate {
    const gateConfig = {
      ...
      petitionProposalCloutUrl: this.config.petitionProposalCloutUrl,
      petitionProposalTrustHops: this.config.petitionProposalTrustHops,
      petitionProposalDelegates: this.config.petitionProposalDelegates,
      petitionProposalScarcityUrl: this.config.petitionProposalScarcityUrl,
      petitionProposalTokenId: this.config.petitionProposalTokenId,
      petitionProposalMinAmount: this.config.petitionProposalMinAmount,
    };
  ...
  private createVoterGate(): VoterGate {
    const gateConfig = {
      ...
      ballotGateAdminKey: this.config.ballotGateAdminKey ?? this.identity.publicKey,
      voterGate: this.config.voterGate ?? 'freebird',
      voterGateTrustHops: this.config.voterGateTrustHops,
      voterGateCloutUrl: this.config.voterGateCloutUrl,
      voterGateTokenId: this.config.voterGateTokenId,
...
export function createPrestige(configOverrides?: Partial<PrestigeConfig>): Prestige {
  const config: PrestigeConfig = {
    ...
    ballotGateTrustHops: process.env.BALLOT_GATE_TRUST_HOPS
      ? parseInt(process.env.BALLOT_GATE_TRUST_HOPS, 10)
      : 2,
    ...
    voterGateTrustHops: process.env.VOTER_GATE_TRUST_HOPS
      ? parseInt(process.env.VOTER_GATE_TRUST_HOPS, 10)
      : 3,
    voterGateCloutUrl: process.env.VOTER_GATE_CLOUT_URL,
    voterGateTokenId: process.env.VOTER_GATE_TOKEN_ID,
    voterGateMinAmount: process.env.VOTER_GATE_MIN_AMOUNT
      ? parseInt(process.env.VOTER_GATE_MIN_AMOUNT, 10)
      : 1,
    ...
    petitionProposalCloutUrl: process.env.PETITION_PROPOSAL_CLOUT_URL,
    petitionProposalTrustHops: process.env.PETITION_PROPOSAL_TRUST_HOPS
      ? parseInt(process.env.PETITION_PROPOSAL_TRUST_HOPS, 10)
      : 1,
    petitionProposalDelegates: process.env.PETITION_PROPOSAL_DELEGATES?.split(',').filter(Boolean),
    petitionProposalScarcityUrl: process.env.PETITION_PROPOSAL_SCARCITY_URL,
    petitionProposalTokenId: process.env.PETITION_PROPOSAL_TOKEN_ID,
    petitionProposalMinAmount: process.env.PETITION_PROPOSAL_MIN_AMOUNT
      ? parseInt(process.env.PETITION_PROPOSAL_MIN_AMOUNT, 10)
      : 1,
