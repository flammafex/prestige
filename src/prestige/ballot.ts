...
export class BallotManager {
  ...
  async createBallot(
    ...
    const attestation = await this.witness.attest(
      Crypto.hashObject(ballot),
      now
    );
  ...
  private validateEligibilityConfig(config: EligibilityConfig): void {
    const validTypes = ['open', 'invite-list', 'clout-trust-graph', 'scarbucks', 'allowlist'];
    ...
    if (config.type === 'clout-trust-graph') {
      if (config.trustHops === undefined || config.trustHops < 1) {
        throw new PrestigeValidationError(
          'Trust graph eligibility requires trustHops >= 1'
        );
      }
      if (config.trustHops > 5) {
        throw new PrestigeValidationError('Trust hops cannot exceed 5');
      }
    }

    if (config.type === 'scarbucks') {
      if (!config.tokenId) {
        throw new PrestigeValidationError(
          'Scarbucks eligibility requires tokenId'
        );
      }
      if (config.minAmount !== undefined && config.minAmount < 0) {
        throw new PrestigeValidationError(
          'Scarbucks minAmount cannot be negative'
        );
      }
    }
