...
export interface VoteManagerConfig {
  /** Clout URL for trust graph checks */
  cloutUrl?: string;
  /** Scarcity URL for token ownership checks */
  scarcityUrl?: string;
}
...
export class VoteManager {
  ...
  async castVote(request: CastVoteRequest): Promise<Vote> {
    ...
    const attestation = await this.witness.attest(voteHash, now);

    // 9. Verify attestation timestamp is within ballot window
    if (attestation.timestamp > ballot.deadline) {
  ...
  async validateVote(vote: Vote, ballot: Ballot): Promise<ValidationResult> {
    ...
    if (vote.attestation.timestamp > ballot.deadline) {
  ...
  private async checkEligibility(ballot: Ballot, publicKey: string): Promise<GateResult> {
    switch (ballot.eligibility.type) {
      ...
      case 'clout-trust-graph':
        // Must be within N hops of creator
        return this.checkCloutEligibility(ballot, publicKey);

      case 'scarbucks':
        // Must hold required tokens
        return this.checkScarbucksEligibility(ballot, publicKey);
  ...
  /**
   * Check Clout trust graph eligibility
   */
  private async checkCloutEligibility(ballot: Ballot, publicKey: string): Promise<GateResult> {
    if (!this.config.cloutUrl) {
      console.warn('Clout URL not configured, treating clout-trust-graph as open');
      return { allowed: true };
    }

    const maxHops = ballot.eligibility.trustHops ?? 3;
    const creatorKey = ballot.eligibility.creatorPublicKey ?? ballot.creatorPublicKey;

    // Self is always 0 hops
    if (publicKey === creatorKey) {
      return { allowed: true };
    }

    try {
      const response = await fetch(`${this.config.cloutUrl}/api/trust/distance`, {
        method: 'POST',
      ...
      if (distance < 0) {
        return { allowed: false, reason: 'Not in the trust network' };
      }

      if (distance > maxHops) {
        return {
          allowed: false,
          reason: `Beyond ${maxHops} trust hops from ballot creator (distance: ${distance})`,
        };
      }

      return { allowed: true };
    } catch (error) {
      console.error('Clout trust check failed:', error);
      return { allowed: false, reason: 'Failed to verify trust graph eligibility' };
    }
  }

  /**
   * Check Scarbucks token eligibility
   */
  private async checkScarbucksEligibility(ballot: Ballot, publicKey: string): Promise<GateResult> {
    if (!this.config.scarcityUrl) {
      console.warn('Scarcity URL not configured, treating scarbucks as open');
      return { allowed: true };
    }

    const tokenId = ballot.eligibility.tokenId;
    const minAmount = ballot.eligibility.minAmount ?? 1;

    if (!tokenId) {
      return { allowed: false, reason: 'Token ID not specified in ballot eligibility' };
    }

    try {
      const response = await fetch(`${this.config.scarcityUrl}/api/tokens/verify-ownership`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tokenId,
      ...
      return {
        allowed: false,
        reason: `Requires ${minAmount}+ ${tokenId} tokens`,
      };
    } catch (error) {
      console.error('Scarbucks token check failed:', error);
      return { allowed: false, reason: 'Failed to verify token ownership' };
    }
  }
}
